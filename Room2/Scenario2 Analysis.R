...
r2_d1pxl = read.table("room2_st1pxl466") # Individuals with a radius of 1 pixel
r2_d12pxl = read.table("room2_st12pxl466") # Individuals with a radius of 12 pixels
r2_d15pxl = read.table("room2_st15pxl466") # Individuals with a radius of 15 pixels
r2_drpxl = read.table("room2_rpxl466") # Individuals with a random uniform radius between 9 and 15
...
y1 = r2_d1pxl[-c(seq(74, 748+75, by = 75), seq(75, 749+75, by = 75)), ]
y1 = y1[y1[,1]>0, ]
...
y3 = r2_d15pxl[-c(seq(74, 748+75, by = 75), seq(75, 749+75, by = 75)), ]
y3 = y3[y3[,1]>0, ]
...
y4 = r2_drpxl[-c(seq(74, 748+75, by = 75), seq(75, 749+75, by = 75)), ]
y4 = y4[y4[,1]>0, ]
y4 = summarySE(y4, measurevar = "exit_pp", groupvars=c("rank"))
y4 = cbind(y4, cn = rep(4, 72))
...
ggplot(data = fdata, mapping = aes(x=rank, y=exit_pp, group = cn, ymin=exit_pp-se, ymax=exit_pp+se)) + geom_line() + 
  ...
  ylim(0,70)+xlim(0,72)+ggtitle("Outflow with an obstacle") + 
...
library(BSDA)
mean(r1_d1pxl$exit_group)-mean(r1_d15pxl$exit_group)
...
SIGN.test(unique(r2_d12pxl[[2]]), unique(r2_drpxl[[2]]))
mean(y2$exit_pp[72])-mean(y4$exit_pp[72])
SIGN.test(unique(r2_d12pxl[[2]]), unique(r2_d15pxl[[2]]))
mean(y3$exit_pp[72])-mean(y4$exit_pp[72])
SIGN.test(unique(r2_drpxl[[2]]), unique(r2_d15pxl[[2]]))